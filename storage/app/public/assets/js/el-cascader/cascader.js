;(function ($) {
    ...
    ZdCascader.prototype._loadChildren = function ($parentNode) {
        ...
        $.each(data.children, (i, m) => {
            ...
            else if (selectedValue == m.value || selectedId.indexOf(m.value) >= 0) {
    ...
    ZdCascader.prototype._getPathData = function ($node) {
        var path = [];
        var $current = $node;
        while ($current && $current.length > 0) {
            var data = $current.data('bindData');
            if (data) path.unshift(data);
            var $wrap = $current.closest('.' + this.CLASS.menuWrap);
            var $prevWrap = $wrap.prev();
            if ($prevWrap.length === 0) break;
            $current = $prevWrap.find('li.' + this.CLASS.checkClass.menuNodeSelected).eq(0);
            if ($current.length === 0) {
                $current = $prevWrap.find('li.' + this.CLASS.checkClass.nodeAnchor).eq(0);
            }
        }
        return path;
    }

    ZdCascader.prototype._getPathLabel = function (pathData) {
        return $.map(pathData || [], function (item) {
            return item.label;
        }).join(this.options.range);
    }

    ZdCascader.prototype._setSelection = function ($node) {
        var data = $node.data('bindData');
        var allPathData = this._getPathData($node);
