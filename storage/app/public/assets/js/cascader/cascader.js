...
layui.define(["jquery"], function (exports) {
  ...
  function getDefaultConfig() {
    return {
      ...
      addNode: {
        enabled: false,      // 是否开启每级新增按钮
        position: 'last',    // 新增按钮位置：first/last
        text: '+ 新增',      // 新增按钮文本
        onAdd: null          // 新增回调 function(ctx){}
      },
      props: {
  ...
  Cascader.prototype = {
    ...
    _initPanel: function () {
      ...
      if (!$panel) {
        ...
        this.$panel = $('<div class="el-popper el-cascader__dropdown ' + popperClass + '" style="position: absolute; z-index: 19891014;display: none;" x-placement="bottom-start"><div class="el-cascader-panel"></div><div class="popper__arrow" style="left: 35px;"></div></div>');
        $panel = this.$panel;
        $panel.appendTo('body');
    ...
    _appendMenu: function (nodes, level, parentNode, _menuItem) {
      ...
      // 渲染细项
      this._appendLi($div, nodes, level, parentNode);
     ...
     * @private
     */
    _appendAddNode: function ($menu, level, parentNode) {
      var addNodeConfig = this.config.addNode || {};
      if (!addNodeConfig.enabled) {
        return;
      }
      var text = addNodeConfig.text || '+ 新增';
      var $list = $menu.find('.el-cascader-menu__list');
      var $li = $('<li role="menuitem" class="el-cascader-node el-cascader-node--add" tabindex="-1"><span class="el-cascader-node__label">' + text + '</span></li>');
      var self = this;
      $li.on('click', function (event) {
        stopPropagation(event);
        var fn = addNodeConfig.onAdd;
        if (typeof fn === 'function') {
          fn({
            cascader: self,
            level: level,
            parentNode: parentNode || null,
            parentValue: parentNode ? parentNode.value : null,
            parentLabel: parentNode ? parentNode.label : null
          });
        }
      });
      if (addNodeConfig.position === 'first') {
        $list.prepend($li);
      } else {
        $list.append($li);
      }
     ...
     * @param nodeData
     */
    addNodeAt: function (level, parentNode, nodeData) {
      var childrenKey = this.props.children;
      var targetArray = parentNode ? (parentNode.data[childrenKey] || []) : this.config.options;
      if (!Array.isArray(targetArray)) {
        targetArray = [];
      }
      targetArray.push(nodeData);
      if (parentNode) {
        parentNode.data[childrenKey] = targetArray;
        parentNode.setChildren(this.initNodes(targetArray, level + 1, parentNode));
      } else {
        this.config.options = targetArray;
        this.data.nodes = this.initNodes(targetArray, 0, null);
      }
      this.refreshMenu();
      if (parentNode && typeof parentNode.expandPanel === 'function') {
        parentNode.expandPanel();
      }
    },
    /**
     * 更新某层级的节点列表
     ...
     * @param parentNode
     */
    refreshLevel: function (level, parentNode) {
      var nodes = parentNode ? parentNode.getChildren() : this.data.nodes;
      this._appendMenu(nodes, level, parentNode, this.data.menuData[level]);
    },
    /**
     * 移除菜单
     * @param level
     ...
     * @param nodes  当前层级数据
     * @param level 当前层级
     * @param parentNode 当前层级父节点
     * @private
     */
    _appendLi: function ($menu, nodes, level, parentNode) {
      var $list = $menu.find('.el-cascader-menu__list');
      if (!nodes || nodes.length === 0) {
        var isEmpty = this.config.empty;
        $list.append('<div class="el-cascader-menu__empty-text">' + isEmpty + '</div>');
        this._appendAddNode($menu, level, parentNode);
        return;
      }
